...
export const POST: RequestHandler = async ({ locals, params }) => {
  ...
  await db.execute({
    sql: 'UPDATE accounts SET locked_by_dealer = 0, next_payment_due = ?, updated_at = ? WHERE id = ?',
    args: [now + DAY_MS, Math.floor(now / 1000), accountId]
  });

  await db.execute({
    sql: "INSERT INTO lock_events (id, account_id, event_type, triggered_by, created_at) VALUES (?, ?, 'unlock', 'dealer', ?)",
