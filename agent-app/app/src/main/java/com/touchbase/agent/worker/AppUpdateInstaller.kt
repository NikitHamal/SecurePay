...
object AppUpdateInstaller {
    ...
    suspend fun downloadVerifyAndInstall(
        ...
        try {
            ...
            val tempFile = File(context.cacheDir, "agent_update_${System.nanoTime()}.apk")
            try {
                ...
                abandonStaleSessions(installer, context.packageName)
    ...
    private fun abandonStaleSessions(installer: PackageInstaller, packageName: String) {
        for (sessionInfo in installer.allSessions) {
            if (sessionInfo.appPackageName == packageName) {
                runCatching { installer.abandonSession(sessionInfo.sessionId) }
            }
        }
    }

    private const val TAG = "AgentAppUpdateInstaller"
    private const val ACTION_INSTALL_COMMIT = "com.touchbase.agent.action.APP_UPDATE_COMMIT"
}
