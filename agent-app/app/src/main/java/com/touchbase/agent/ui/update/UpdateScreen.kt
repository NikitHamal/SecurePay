package com.touchbase.agent.ui.update
...
import com.touchbase.agent.data.model.AppUpdateResponse
import com.touchbase.agent.data.remote.SecurePayRepository
import com.touchbase.agent.ui.theme.Amber
import com.touchbase.agent.ui.theme.GoldGreen
import com.touchbase.agent.ui.theme.OnDarkPrimary
import com.touchbase.agent.ui.theme.OnDarkSecondary
import com.touchbase.agent.ui.theme.GoldDim
import com.touchbase.agent.worker.AppUpdateInstaller
...
fun UpdateScreen(
    ...
    Scaffold(
        ...
        topBar = {
            TopAppBar(
                title = { Text("App Updates", fontWeight = FontWeight.Bold, color = MaterialTheme.colorScheme.onBackground) },
                navigationIcon = {
        ...
        Column(
            ...
            Card(
                ...
                Column(
                    ...
                    Box(
                        modifier = Modifier.size(76.dp).background(GoldGreen.copy(alpha = 0.16f), RoundedCornerShape(24.dp)),
                        contentAlignment = Alignment.Center
                    ) {
                        Icon(Icons.Filled.SystemUpdate, contentDescription = null, tint = GoldGreen, modifier = Modifier.size(42.dp))
                    }
                    Text("Manual Update Check", style = MaterialTheme.typography.headlineSmall, fontWeight = FontWeight.Bold, color = MaterialTheme.colorScheme.onBackground)
                    Text(
                        "Checks your dealer server for a newer Touch Base Agent APK. If an update is available, the app downloads it, verifies its SHA-256 checksum, then asks Android Package Installer to install it.",
            ...
            message?.let {
                Text(it, color = if (update?.available == true) Amber else GoldGreen, style = MaterialTheme.typography.bodyMedium, textAlign = TextAlign.Center)
            }

            if (phase != UpdatePhase.IDLE) {
                CircularProgressIndicator(color = GoldGreen)
                Text(if (phase == UpdatePhase.CHECKING) "Checking server..." else "Downloading and verifying APK...", color = MaterialTheme.colorScheme.onSurfaceVariant)
            }
            ...
            Button(
                ...
                colors = ButtonDefaults.buttonColors(containerColor = GoldGreen, contentColor = Color(0xFF07130F))
            ) {
...
private fun VersionRow(label: String, value: String) {
    Row(modifier = Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween) {
        ...
        Text(value, color = MaterialTheme.colorScheme.onBackground, style = MaterialTheme.typography.bodyMedium, fontWeight = FontWeight.SemiBold)
    }
}
